...
class SpectrogramGenerator:
    def __init__(self, CSI_data):
        ...
        self.time_total = np.arange(N_packets) / fs  # Time vector: each data stamp is taken at a frequncy of 500Hz (assumed from the file being labled 500)
        self.duration = time_total[-1] # gets last index meanign total duration of the signal
    ...
    def prep_CSI_data(self, raw_CSI_data):
        ...
        signal_filtered = bandpass_filter(signal_clean, fs, low=self.f_min, high=self.f_max, order=4)
...
f, t, mag = sf.compute_STFT(csi_post_pca, fs, T_win=0.4)
