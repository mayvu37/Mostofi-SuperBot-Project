...
def classify_latest_row_csv(filename, k=5):
	...
	x_test = test_row.iloc[1:].values.reshape(1,-1)
...
def update_feature_file(feature_vector, data_name, filename='features.csv'):
    """
    Saves the feature vector to a CSV file. 
    If the file is missing or empty, it creates it with headers.
    ...
    # 2. Build the row
    # row = [Name] + [35-36 feature numbers] + [Source Filename]
    row = [label] + list(feature_vector) + [data_name]

    # 3. Define the columns
    # This must match the length of 'row' exactly
    columns = (["Label", "torso_avg", "torso_range", "cycle_time", "stride_length",
                "freq_dist_variance", "freq_dist_max"]
                + [f"velocity_bin_{i}" for i in range(len(list(feature_vector)) - 6)]
                + ["Data Name"])

    # 4. Create the DataFrame for the new row
    try:
        df_new = pd.DataFrame([row], columns=columns)
    except ValueError as e:
        print(f"Error: Column mismatch. Row length: {len(row)}, Headers length: {len(columns)}")
        print(f"Details: {e}")
        return

    # 5. Save logic (Check if file exists AND has content)
    # This prevents the 'no headers' issue if a blank file was created previously
    file_is_valid = os.path.exists(filename) and os.path.getsize(filename) > 0

    if not file_is_valid:
        # Create the file and write headers (index=False prevents extra 0,1,2 column)
        df_new.to_csv(filename, index=False)
        print(f"Created new file '{filename}' with headers and saved data for '{label}'.")
    else:
        # Append to existing file, do NOT write headers again
        df_new.to_csv(filename, mode='a', header=False, index=False)
        print(f"Feature vector appended for '{label}' to '{filename}'.")
#=====================================================================================================    
